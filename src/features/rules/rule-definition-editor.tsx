  ...
  return <section className="space-y-4"><h3 className="font-semibold">HTTP 规则内容</h3>
    ...
    <section className="space-y-2 rounded-lg border border-[var(--telemetry-line)] p-3"><h4 className="font-medium">HTTP Body Document</h4>
      {value.document ? <>
        ...
        <DocumentEditor
          actions={value.document.actions}
          commonActions={props.stage?.document_common_actions ?? []}
          conditions={value.document.conditions}
          editorScope={scope}
          fields={props.stage?.document_fields ?? []}
          key={`${scope}:document`}
          packageLabel={`${value.document.package.id}@${value.document.package.version}`}
          onActionsChange={(actions) => update({ ...value, document: { ...value.document!, actions } })}
          onConditionsChange={(conditions) => update({ ...value, document: { ...value.document!, conditions } })}
...
function HttpFactoryControls(props: {
  ...
  actions: RuleAction[];
  conditions: import("@/generated/rust-types").MatchCondition[];
  editorScope: string;
  stage: import("@/generated/rust-types").MessageStage;
  onChange: (conditions: import("@/generated/rust-types").MatchCondition[], actions: RuleAction[]) => void;
  ...
  return <div className="space-y-2">
    ...
    {props.conditions.map((condition, index) => <div className="flex items-center rounded-md border p-2 text-xs" key={index}><span>{httpConditionLabel(condition)}</span><Button className="ml-auto" size="sm" variant="ghost" onPress={() => props.onChange(props.conditions.filter((_, itemIndex) => itemIndex !== index), props.actions)}>删除</Button></div>)}
    {props.actions.map((action, index) => <div className="flex items-center rounded-md border p-2 text-xs" key={index}><span>{httpRuleActionLabel(action)}</span><Button className="ml-auto" size="sm" variant="ghost" onPress={() => props.onChange(props.conditions, props.actions.filter((_, itemIndex) => itemIndex !== index))}>删除</Button></div>)}
...
function SocketContentEditor(props: { input: RuleDefinitionSaveInput; stage?: SocketRuleEditorStage; onChange: (change: RuleDefinitionChange) => void }) {
  ...
  return <section className="space-y-3"><h3 className="font-semibold">Socket Document 规则内容</h3>
    <DocumentEditor actions={value.actions} commonActions={props.stage?.common_actions ?? []} conditions={value.conditions} editorScope={editorScope(props.input)} fields={props.stage?.fields ?? []} key={`${editorScope(props.input)}:document`} packageLabel={`${value.package.id}@${value.package.version}`} onActionsChange={(actions) => props.onChange({ ...props.input, draft: { ...props.input.draft, content: { type: "socket", value: { ...value, actions } } } })} onConditionsChange={(conditions) => props.onChange({ ...props.input, draft: { ...props.input.draft, content: { type: "socket", value: { ...value, conditions } } } })} onCreateAction={(action) => props.onChange((current) => appendDocumentResult(current, editorScope(props.input), "action", action))} onCreateCondition={(condition) => props.onChange((current) => appendDocumentResult(current, editorScope(props.input), "condition", condition))} />
  </section>;
}
...
function DocumentEditor(props: { packageLabel: string; editorScope: string; fields: ProtocolRuleFieldCapability[]; commonActions: ProtocolRuleCommonActionCapability[]; conditions: DocumentCondition[]; actions: DocumentAction[]; onCreateCondition: (condition: DocumentCondition) => void; onCreateAction: (action: DocumentAction) => void; onConditionsChange: (conditions: DocumentCondition[]) => void; onActionsChange: (actions: DocumentAction[]) => void }) {
  ...
  const createWithValue = (field: ProtocolRuleFieldCapability, kind: "condition" | "set_field") => {
    ...
    void runAsync(`${kind}:${field.name}`, () => callCommand(commands.ruleParseDocumentValue(field.type, raw)), (value) => {
      if (kind === "condition") props.onCreateCondition({ operator: "equals", field: field.name, value });
      else props.onCreateAction({ type: "set_field", field: field.name, value });
    }, (reason) => setError(errorMessage(reason)));
  };
  return <div className="space-y-2"><p className="text-sm font-medium">{props.packageLabel}</p><p className="text-xs text-[var(--telemetry-muted)]">字段 {props.fields.length} 个 · 条件 {props.conditions.length} 个 · Document 动作 {props.actions.length} 个</p>
    {props.fields.map((field) => <div className="space-y-2 rounded-md border p-2" key={field.name}><TextField><Label>Document 值：{field.label}</Label><Input aria-label={`Document 值：${field.label}`} value={rawValues[field.name] ?? ""} onChange={(event) => setRawValues((current) => ({ ...current, [field.name]: event.target.value }))} /></TextField><div className="flex flex-wrap gap-2">
      ...
      {field.actions.includes("clear_field") && <Button isDisabled={pending} size="sm" variant="outline" onPress={() => props.onCreateAction({ type: "clear_field", field: field.name })}>添加动作：Clear {field.label}</Button>}
    </div></div>)}
...
function appendHttpFactoryResult(
  ...
  const next = kind === "condition"
    ? { ...content, conditions: [...content.conditions, value as import("@/generated/rust-types").MatchCondition] }
    : { ...content, actions: [...content.actions, value as RuleAction] };
  return { ...input, draft: { ...input.draft, content: { type: "http" as const, value: next } } };
}

function appendDocumentResult(input: RuleDefinitionSaveInput, expectedScope: string, kind: "condition" | "action", value: DocumentCondition | DocumentAction) {
  ...
  if (content.type === "http") {
    ...
    const document = kind === "condition"
      ? { ...content.value.document, conditions: [...content.value.document.conditions, value as DocumentCondition] }
      : { ...content.value.document, actions: [...content.value.document.actions, value as DocumentAction] };
    return { ...input, draft: { ...input.draft, content: { type: "http" as const, value: { ...content.value, document } } } };
  }
  const next = kind === "condition"
    ? { ...content.value, conditions: [...content.value.conditions, value as DocumentCondition] }
    : { ...content.value, actions: [...content.value.actions, value as DocumentAction] };
