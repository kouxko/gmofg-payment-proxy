...
describe("SocketProcessingCard", () => {
  ...
  it("selecting a package persists only its exact identity and enables the full chain", async () => {
    ...
    await user.click(screen.getByLabelText("Socket 协议处理方案"));
    await user.click(await screen.findByRole("option", { name: "ISO 8583 1.0.0 · 1.0.0 · 远端调试 · 在线" }));
  ...
  it("lists only authoritative exact versions", async () => {
    ...
    await user.click(screen.getByLabelText("Socket 协议处理方案"));
    expect(await screen.findByRole("option", { name: "ISO 8583 1.0.0 · 1.0.0 · 远端调试 · 在线" })).toBeVisible();
    expect(screen.getByRole("option", { name: "ISO 8583 2.0.0 · 2.0.0 · 远端调试 · 在线" })).toBeVisible();
