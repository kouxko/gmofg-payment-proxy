          ...
          <Card>
            <Card.Content className="grid grid-cols-2 gap-4 p-5 max-[700px]:grid-cols-1">
              ...
              {selected.kind === "reverse" ? (
                ...
                <>
                  ...
                  <div className="col-span-2 grid grid-cols-2 gap-4 rounded-2xl border border-[var(--telemetry-line)] p-4 max-[700px]:col-span-1 max-[700px]:grid-cols-1">
                    <Switch isSelected={selected.authentication.mode === "basic"} onChange={(enabled) => replaceForward({ authentication: enabled ? { mode: "basic", credential: { provider: "system", key: "" } } : { mode: "none" } })}><Switch.Control><Switch.Thumb /></Switch.Control><Switch.Content>启用 HTTP Basic 认证</Switch.Content></Switch>
                    <Switch isSelected={selected.mitm.enabled} onChange={(enabled) => replaceForward({ mitm: { ...selected.mitm, enabled } })}><Switch.Control><Switch.Thumb /></Switch.Control><Switch.Content>启用 allowlist MITM</Switch.Content></Switch>
                    {basicCredential && <>
              ...
              {selected.kind === "reverse" && <div className="col-span-2 grid grid-cols-2 gap-4 rounded-2xl border border-[var(--telemetry-line)] p-4 max-[700px]:col-span-1 max-[700px]:grid-cols-1">
                <Switch isSelected={selected.downstream_tls.enabled} onChange={(enabled) => replaceReverse({ downstream_tls: { ...selected.downstream_tls, enabled } })}><Switch.Control><Switch.Thumb /></Switch.Control><Switch.Content>下游 TLS / mTLS</Switch.Content></Switch>
                <Switch isSelected={selected.upstream_tls.verify_hostname} onChange={(verify_hostname) => replaceReverse({ upstream_tls: { ...selected.upstream_tls, verify_hostname } })}><Switch.Control><Switch.Thumb /></Switch.Control><Switch.Content>校验上游主机名</Switch.Content></Switch>
