...
1. 先读取入口、运行状态、端点、证书概览、抓包和诊断。
2. 明确失败发生在 App→代理、代理处理、代理→上游或返回路径。
3. 给出一个最小修改方案，并补充 1 至 2 个合理替代方案。
4. 每个方案说明影响范围、风险、回退方法和可观察的验证结果。
5. 如果证据不足，列出需要补充的日志、抓包字段或 App 配置，不把猜测写成结论。
