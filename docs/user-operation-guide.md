...
- **导出 Workspace**：导出当前 Workspace 的全部可移植配置，不导出私钥、P12、密码或抓包 Payload。
...
5. 如客户端连接本地代理时要求 TLS，选择下游服务端证书引用。客户端只要求普通 HTTPS 时，把下游客户端
...
7. 每条入口独立导入或选择证书引用。例如交易入口与 DLL 入口可以使用不同的下游证书、
   上游客户端身份和上游 CA；安全材料不会写入 Workspace。
8. 使用系统信任或在当前入口点击“导入 CA”，并保持主机名校验开启。
9. 在每个代理入口中直接选择请求正文编码和响应正文编码：原始字节、UTF-8 或 Shift-JIS。
10. 点击“测试 Server TLS 握手”。应用会先校验并保存当前 Workspace，再由 Rust 真实连接该
...
- 需要迁移整台工具的配置时，使用“导出整个应用配置”生成 `.intercept-config`。
  新安装导入后仍需在本机重新导入/选择密钥和证书材料；配置文件只保留安全引用，不携带秘密。
