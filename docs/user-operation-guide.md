...
匹配条件、对应动作和保存按钮。元数据或条件/动作不完整时保存按钮保持禁用。选择规则或新建草稿
不会自动保存，规则编辑本身不使用弹窗。
...
- HTTP 动作参数按动作显示中文字段和单位，不需要手写 JSON；没有参数的动作不显示参数区域；
- 切换 HTTP 动作会清空上一动作的参数，必填字段未填写完整时不能保存；
...
HTTP 和 Socket 同时显示在一个页面，不通过协议 Tab 隐藏另一类记录。连接按建立时间从新到旧分页，
最新连接显示在第一页最上方；单个 Exchange 内的事件仍按实际发生顺序展示。
...
4. 在“常用弱网效果”中选择参考 2G、参考慢速 3G、参考慢速 4G、完全断网，或选择自定义后填写
   延迟、延迟波动、丢包率和上下行限速。弱网可以单独保存并启动，不要求配置桌面代理入口。
...
参考 2G/慢速 3G 使用 [sitespeed.io Throttle](https://www.sitespeed.io/documentation/throttle/) 的公开
预设；参考慢速 4G 使用 [Google Lighthouse](https://github.com/GoogleChrome/lighthouse/blob/main/docs/throttling.md)
的移动网络预设。本项目按十进制 Kbps 除以 8 转换为 B/s，并把公开 RTT 除以 2 写入双向逐包固定延迟。
这些是可重复的实验室参考条件，不代表特定运营商、地区或真实地铁/电梯轨迹。
