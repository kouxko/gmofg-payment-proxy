...
| --- | --- | --- | --- | --- | --- |
| 18:13:45 +08:00 | [修复分块 HTTP 响应转发时的长度头冲突](completed/2026-09-04/fix-http-chunked-response-forwarding.md) | 保留上游 transfer-coding，由 Hyper 重生合法 chunk framing，不添加冲突 Content-Length；同步保持状态码改写、截断故障和 EOF 定界复合 coding 的精确语义 | PASS；Proxy 236/236、fmt、严格 Clippy、ESLint、diff check 与独立复审通过，P0/P1/P2=0 | 高 | HTTP/1.1、Keep-Alive、Transfer-Encoding、Content-Length、chunked、gzip、404 |
| 14:26:46 +08:00 | [将 HTTP 动作参数改为明确表单](completed/2026-09-04/http-action-parameter-forms.md) | 移除原始“动作参数 JSON”，按动作显示中文数值字段、单位和枚举下拉；切换动作清空旧参数，无参动作完全隐藏参数区域，并保持 Rust factory 合同 | PASS_WITH_INSTALLED_UI_CONTENT_AUTOMATION_NOT_RUN_AND_KNOWN_UNRELATED_SOURCE_SIZE_FAILURE；前端 566/566、factory 1/1、typecheck/lint/架构/绑定/构建/签名/安装通过；WebView 内容自动化 NOT_RUN，源码尺寸仅 7 个既有无关超限 | 高 | HTTP 规则、动作参数、延迟、限速、故障注入、表单、macOS 安装 |
| 14:17:58 +08:00 | [修复证书记录与聚合修订号语义冲突](completed/2026-09-04/fix-certificate-revision-semantics.md) | 区分证书集合 CAS revision 与单条记录 revision；集合推进后未变化的固定 Root/Leaf 仍可通过状态门禁，解密时继续校验记录元数据与受保护材料版本一致 | PASS；回归测试先 RED 后 GREEN，Infrastructure 513/513、fmt、strict Clippy 和 diff check 全部通过 | 高 | 证书、certificate_state、metadata revision、SQLite、启动门禁 |
