...
| 持久运行日志与故障复现报告 | `src-tauri/crates/application/src/models/diagnostic_report.rs` | `src-tauri/crates/application/src/facade/diagnostic_report.rs` | `src-tauri/src/runtime_logs`, `src-tauri/src/reproduction_report.rs` | `src-tauri/src/commands/diagnostics.rs`, `src/features/diagnostics/diagnostic-logs-view.tsx`, MCP `reproduction_report` | `src-tauri/crates/application/src/requirements_tests/diagnostic_report.rs`, `src-tauri/src/commands/e2e_tests/mod.rs`, `src/features/diagnostics/diagnostic-logs-view.test.tsx` |
