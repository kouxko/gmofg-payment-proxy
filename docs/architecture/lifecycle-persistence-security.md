...
flowchart LR
  ...
  Workspace --> DB[(SQLite current schema)]
  Workspace --> Portable[application backup ZIP]
...
| Workspace -> SQLite current schema | `application_schema` 只接受当前 schema；不兼容数据由启动恢复流程清空后按当前 schema 重建 | `src-tauri/crates/infrastructure/src/sqlite/schema.rs`, `src-tauri/crates/infrastructure/src/sqlite/core.rs` | `src-tauri/crates/infrastructure/src/sqlite/tests/workspace_and_settings.rs`, `src-tauri/crates/infrastructure/src/sqlite/tests/protocol_packages.rs` |
| application backup ZIP | application 定义严格 v1 wire 与 prepare/preview/commit；infrastructure 负责受限 ZIP、原子写入和一次性 token | `src-tauri/crates/application/src/application_backup.rs`, `src-tauri/crates/application/src/backup_import.rs`, `src-tauri/crates/infrastructure/src/application_backup.rs` | `src-tauri/crates/application/src/requirements_tests/workspace_configuration/application_backup_import.rs`, `src-tauri/crates/infrastructure/tests/r07c_application_backup_import_tokens.rs` |
...
- [ADR-004](decisions/ADR-004-embedded-read-only-mcp.md) 规定本机只读 MCP 的 loopback 信任边界、只读 Application facade、资源预算和有界关闭。
