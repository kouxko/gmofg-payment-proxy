...
当前开发启动路径仍把合法单行 Schema 标记 `<100` 的数据库认定为开发数据。Infrastructure 在一个
SQLite 事务中删除全部旧用户 Schema 和数据，再按版本 `100` 重建；缺失、重复或损坏标记不能触发
清理。版本 `100` 数据原样保留；当前程序遇到 `>100` 的未来数据库时 fail-closed，不能改写。该重建
路径是 Phase17 必须删除的发布阻塞，不是产品兼容、迁移或恢复合同。
