...
///
/// 实现按持久化载荷返回 `Managed` 或 `External`，并将 `online` 作为当前可调用状态快照。
/// 首次合法远端注册立即持久化为 enabled；断线只改变 online 投影，不得
/// 自动重连远端包或 Listener。所有方法都按精确 `(package_id, version)` 操作，不得自动
/// 选择同 ID 的其他版本。
pub trait ExternalPackageApplicationPort: Send + Sync + std::fmt::Debug {
    ...
    /// 返回注册时严格校验并持久化的安全描述，不执行远端网络 RPC。
    async fn describe(
        &self,
        package: &ProtocolPackageRef,
    ...
    /// 原子写入用户启用位。本地 Component 启用时必须实例化 exact runtime，停用时必须回收；
    /// 远端软件包只修改启用位，不接管其进程或连接。
    async fn set_enabled(&self, package: &ProtocolPackageRef, enabled: bool) -> AppResult<()>;

    /// 仅重新实例化 Proxy 拥有的本地 exact Component；必须先移除旧实例且不 replay，
    /// 再加载同一持久化 BLOB。远端软件包必须拒绝。
