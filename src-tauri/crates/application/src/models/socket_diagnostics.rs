//! Socket Listener 生命周期、路由与失败的轻量脱敏诊断 DTO。
//!
//! 这些值只描述当前内存中的连接诊断；它们不是报文仓储，也不携带 Document、
//! 原始 Frame 或部分写出字节。
...
/// 已脱敏的 Socket 失败证据。
pub struct SocketFailureDiagnostic {
    pub stage: SocketFailureStage,
    pub code: String,
}
