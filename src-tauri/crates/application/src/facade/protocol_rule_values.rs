...
use intercept_proxy_domain::{
    DocumentValue, MAX_JAVASCRIPT_SAFE_INTEGER, MAX_PROTOCOL_DOCUMENT_RULE_BLOB_BYTES,
    MAX_PROTOCOL_DOCUMENT_RULE_STRING_BYTES,
};

use crate::{AppError, AppResult, ProtocolPackageSchemaFieldTypeViewModel};

const MAX_BLOB_TEXT_BYTES: usize = MAX_PROTOCOL_DOCUMENT_RULE_BLOB_BYTES * 4;
/// Int 的合法文本最多只有 17 个字节；保留 128 字节预算允许常见的外围空白，同时在
/// `trim`、逐字节语法检查和 `i64` 解析前拒绝无意义的超长 IPC 输入。
const MAX_PROTOCOL_RULE_INT_TEXT_BYTES: usize = 128;
...
fn parse_int(raw: &str) -> AppResult<DocumentValue> {
    if raw.len() > MAX_PROTOCOL_RULE_INT_TEXT_BYTES {
...
fn parse_blob(raw: &str) -> AppResult<DocumentValue> {
    ...
    for character in raw.chars() {
        ...
        if let Some(high) = high_nibble.take() {
            if bytes.len() == MAX_PROTOCOL_DOCUMENT_RULE_BLOB_BYTES {
