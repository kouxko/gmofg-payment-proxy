...
impl Application {
    /// 通过宿主原生文件选择器导入 Component；WebView 不提交路径或文件内容。
    pub async fn protocol_package_import(
        ...
        self.protocol_package_importer.prepare_component().await
    }
    ...
    pub async fn protocol_package_import_commit(
        ...
        self.protocol_package_importer.commit_component(token).await
    }
    ...
    pub async fn protocol_package_import_discard(
        ...
        self.protocol_package_importer
            .discard_component(token)
            .await?;
