...
pub(crate) fn validate_http_rule(
    ...
    validate_condition(condition, &mut error);
    validate_action_compatibility(stage, &mut error, action);
    validate_action_limits(&mut error, action);
    validate_action_content(&mut error, action);
...
fn validate_condition(condition: &Condition, error: &mut DomainError) {
    match condition {
            ...
            if let Err(condition_error) = validate_http_condition(field, operator) {
                for (field, messages) in *condition_error.field_errors {
                    for message in messages {
                        push_field_error(error, format!("condition.{field}"), &message);
                    }
                }
            }
        }
...
fn validate_action_compatibility(
    ...
    if !action_compatible(stage, action) {
        push_field_error(error, "action", "动作与规则阶段不兼容");
    }
    if let HttpAction::CustomHttpStatus { status } = action
        && !(100..=599).contains(status)
    {
        push_field_error(error, "action.status", "HTTP 状态码必须位于 100..599");
    }
}

fn validate_action_limits(error: &mut DomainError, action: &HttpAction) {
    match action {
        HttpAction::Delay { milliseconds } if *milliseconds == 0 => {
            push_field_error(error, "action.milliseconds", "延迟必须大于 0 毫秒");
        }
        HttpAction::Jitter {
        ...
        HttpAction::Intermittent {
            ...
            validate_window(error, "available_milliseconds", *available_milliseconds);
            validate_window(error, "blocked_milliseconds", *blocked_milliseconds);
        }
        _ => {}
    }
    ...
    match action {
        HttpAction::Terminal(TerminalAction::IncorrectContentLength { delta }) if *delta == 0 => {
            push_field_error(error, "action.delta", "错误长度差值不能为 0");
        }
        HttpAction::Terminal(
            ...
            push_field_error(error, "action.milliseconds", "故障超时必须大于 0 毫秒");
        }
        HttpAction::Terminal(TerminalAction::MockResponse { status, .. })
            if !(100..=599).contains(status) =>
        {
            push_field_error(error, "action.status", "Mock HTTP 状态码必须位于 100..599");
        }
        _ => {}
    }
}

fn validate_window(error: &mut DomainError, field: &str, value: u64) {
    if !(1..=MAX_TOTAL_DELAY_MS).contains(&value) {
        ...
        push_field_error(error, format!("action.{field}"), message);
    }
}

fn validate_action_content(error: &mut DomainError, action: &HttpAction) {
    match action {
        HttpAction::SetJsonField { path, .. } if JsonPath::parse(path).is_err() => {
            push_field_error(error, "action.path", "JSON 字段路径非法");
        }
        HttpAction::SetHeader { name, value } => {
            validate_header(error, "action", name, value);
        }
        HttpAction::Terminal(TerminalAction::MockResponse { headers, .. }) => {
            for (header_index, (name, value)) in headers.iter().enumerate() {
                validate_header(
                    error,
                    &format!("action.headers.{header_index}"),
                    name,
                    value,
                );
            }
