...
) -> Result<SocketDocumentRuleDefinition, DomainError> {
...
fn schema_validation_rejects_unknown_fields_wrong_types_and_versions() {
    ...
    let versioned: SocketDocumentRuleDefinition = serde_json::from_value(json).unwrap();
...
fn structure_rejects_duplicate_conditions_empty_actions_and_oversize_values() {
    ...
    assert!(
        rule(
            ...
            vec![condition(
                "text",
                DocumentValue::String("x".repeat(MAX_SOCKET_DOCUMENT_RULE_STRING_BYTES + 1)),
            )],
            vec![DocumentAction::RecordMatch],
        )
        .is_err()
    ...
    assert!(
        rule(
            ...
            vec![set(
                "payload",
                DocumentValue::Blob(vec![0; MAX_SOCKET_DOCUMENT_RULE_BLOB_BYTES + 1]),
            )],
        )
        .is_err()
    );
...
fn update_and_toggle_preserve_identity_and_enforce_revision() {
    ...
    assert_eq!(current.direction(), SocketDirection::Upstream);

    let mut switched = current.to_draft();
    switched.direction = SocketDirection::Downstream;
    assert_eq!(
...
fn revision_exhaustion_rejects_update_and_toggle_without_side_effects() {
    ...
    for revision in [
        ...
        let mut update_target = SocketDocumentRuleDefinition {
            revision,
            ..current.clone()
        };
        ...
        let mut toggle_target = SocketDocumentRuleDefinition {
            revision,
            ..current.clone()
        };
...
fn deterministic_sort_uses_priority_created_order_and_rule_id() {
    ...
    let make = |id: u128, priority: i32, created_order: u64| {
        ...
        serde_json::from_value::<SocketDocumentRuleDefinition>(value).unwrap()
    };
    ...
    assert_eq!(
        rules
            ...
            .collect::<Vec<_>>(),
        vec![
            SocketDocumentRuleId::from_uuid(Uuid::from_u128(2)),
            SocketDocumentRuleId::from_uuid(Uuid::from_u128(1)),
            SocketDocumentRuleId::from_uuid(Uuid::from_u128(3)),
        ]
    );
}
