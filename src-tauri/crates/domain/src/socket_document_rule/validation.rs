//! Socket Document 规则的结构与 Schema 校验。
...
pub(super) fn validate_structure(
    ...
    if conditions.len() > MAX_SOCKET_DOCUMENT_RULE_CONDITIONS {
        add_error(&mut error, "conditions", "条件数量不能超过 64 个");
    }
    if actions.is_empty() || actions.len() > MAX_SOCKET_DOCUMENT_RULE_ACTIONS {
...
fn validate_value_limit(value: &DocumentValue, field: &str, error: &mut DomainError) {
    match value {
        DocumentValue::String(value) if value.len() > MAX_SOCKET_DOCUMENT_RULE_STRING_BYTES => {
            add_error(error, field, "文本值不能超过 16 KiB UTF-8 字节");
        }
        DocumentValue::Blob(value) if value.len() > MAX_SOCKET_DOCUMENT_RULE_BLOB_BYTES => {
