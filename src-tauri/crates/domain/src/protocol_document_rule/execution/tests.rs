...
#[test]
fn non_matching_conditions_leave_document_unchanged_and_records_empty() {
    let listener_id = ListenerId::new();
    let candidate = rule(
        1,
        listener_id,
        ...
        10,
        1,
        vec![condition("amount", DocumentValue::Int(10))],
        vec![set("text", DocumentValue::String("changed".into()))],
    );
    let program = program(listener_id, vec![candidate]);
    let mut input = Document::new(schema());
    input
        .set("text", DocumentValue::String("original".into()))
        .unwrap();
    ...
    assert_eq!(
        result.document().get("text").unwrap(),
        &DocumentValue::String("original".into())
    );
}
