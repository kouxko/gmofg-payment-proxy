use super::{
    Condition, ConditionTree, DocumentMutation, DocumentPredicate, DomainError, ErrorCode,
    HttpAction, HttpDocumentRuleContent, MessageStage, ProtocolDocumentOperation,
    ProtocolDocumentPredicate, ProtocolDocumentRuleDefinition, ProtocolDocumentRuleId,
    ProtocolRuleStage, RuleContent, RuleDefinition, RuleStage, SocketRuleContent, UnifiedAction,
};
...
) -> Result<(Vec<Condition>, Vec<HttpAction>), DomainError> {
...
fn collect_legacy_http_conditions(
    ...
    match tree {
        ...
        ConditionTree::Leaf(condition @ Condition::Http { .. }) => {
...
fn legacy_document_parts(
    ...
    (
        Vec<ProtocolDocumentPredicate>,
        Vec<ProtocolDocumentOperation>,
    ),
    DomainError,
> {
    ...
    let actions = actions
        ...
        .filter_map(|action| match action {
            UnifiedAction::Document(DocumentMutation::Set { path, value }) => {
                Some(Ok(ProtocolDocumentOperation::SetField {
                    field: path.clone(),
                    value: value.clone(),
                }))
            }
            ...
            UnifiedAction::RecordMatch => Some(Ok(ProtocolDocumentOperation::RecordMatch)),
...
fn collect_legacy_document_conditions(
    tree: &ConditionTree,
    output: &mut Vec<ProtocolDocumentPredicate>,
) -> Result<(), DomainError> {
    match tree {
        ConditionTree::All(children) => {
            for child in children {
        ...
        ConditionTree::Leaf(Condition::Document { path, predicate }) => {
            ...
            output.push(ProtocolDocumentPredicate::Equals {
                field: path.clone(),
                value,
            });
            Ok(())
...
pub(super) fn unified_http_actions(actions: Vec<HttpAction>) -> Vec<UnifiedAction> {
    actions
        .into_iter()
        .map(|action| match action {
            HttpAction::Terminal(action) => UnifiedAction::Terminal(action),
