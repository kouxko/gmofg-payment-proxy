...
#[test]
fn field_names_accept_protocol_neutral_identifiers_that_are_rhai_keywords() {
    for name in ["fn", "while", "true", "type_of", "is_def_var"] {
        let field = DocumentFieldName::new(name).expect("Domain does not own Rhai policy");
        assert_eq!(field.as_str(), name);
        assert_eq!(
            serde_json::from_str::<DocumentFieldName>(&format!("\"{name}\"")).unwrap(),
            field
        );
    }
}
...
fn schema_and_field_names_support_from_str_display_and_validated_serde() {
    ...
    assert_eq!(
        serde_json::from_str::<DocumentFieldName>("\"while\"")
            .unwrap()
            .as_str(),
        "while"
    );
...
fn schema_deserialization_rejects_unknown_type_unknown_keys_and_invalid_nested_values() {
    ...
    invalid_name["fields"][0]["name"] = serde_json::json!("invalid-name");
