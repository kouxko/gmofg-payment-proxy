...
async fn stores_upstream_security_evidence_on_the_active_session() {
    ...
    open_test_connection(&pipeline, &context).await;
    pipeline
        .apply_request_policy(&context, request_metadata(), &mut request_message(r#"{"amount":100}"#))
        .await
        .unwrap();
    let session_id = pipeline
        .state
...
async fn isolates_interleaved_workspace_metrics_and_ignores_late_stop_events() {
    ...
    pipeline
        .apply_request_policy(&context_a, request_metadata(), &mut request_a)
        .await
        .expect("workspace A request");
    let mut request_b = request_message(r#"{"workspace":"b"}"#);
    pipeline
        .apply_request_policy(&context_b, request_metadata(), &mut request_b)
        .await
        .expect("workspace B request");
    let mut response_b = response_message();
    pipeline
        .apply_response_policy(&context_b, request_metadata(), &mut response_b)
        .await
    ...
    let late_request_error = pipeline
        .apply_request_policy(&late_context_a, request_metadata(), &mut late_request_a)
        .await
