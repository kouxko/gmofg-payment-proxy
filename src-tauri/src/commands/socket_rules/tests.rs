//! `socket_rule_parse_value` 的真实 Tauri IPC 参数与错误映射测试。
...
use super::socket_rule_parse_value;
...
fn ipc_preserves_exact_boundaries_and_returns_payload_free_errors() {
    ...
    assert_eq!(malformed.code, "SOCKET_RULE_VALUE_INVALID");
    ...
    assert_eq!(oversized.code, "SOCKET_RULE_VALUE_TOO_LARGE");
    ...
    assert_eq!(exact_long_digits.code, "SOCKET_RULE_VALUE_INVALID");
    for raw in [format!("{}1", " ".repeat(128)), "1".repeat(129)] {
        let oversized_int = invoke_error(&webview, json!({ "fieldType": "int", "raw": raw }));
        assert_eq!(oversized_int.code, "SOCKET_RULE_VALUE_TOO_LARGE");
...
fn test_app() -> tauri::App<MockRuntime> {
    tauri::test::mock_builder()
        .invoke_handler(tauri::generate_handler![socket_rule_parse_value])
...
fn request(body: Value) -> tauri::webview::InvokeRequest {
    tauri::webview::InvokeRequest {
        cmd: "socket_rule_parse_value".into(),
