...
This test-only gate sends Android instrumentation requests through two independent channels:
DLL uses `http://127.0.0.1:6555`, while Transaction uses `http://127.0.0.1:6556`. Two `adb reverse`
mappings reach two host-side dynamic Reverse Listeners, which forward to different local
Shift-JIS upstream fixture ports. The Rust runner builds the normal
...
11. protocol-equivalent DLL and Transaction requests from the Android shell UID through the
    Companion VpnService, a fixed-delay Rust weak-network profile and the two Reverse Listeners;
12. byte-for-byte Shift-JIS D48 preservation before and during the VPN impairment, with live TUN,
...
`vpn_joint_probe` evidence for the direct and impaired DLL/Transaction paths. The script rejects
...
emulator, `adb reverse`, Reverse Listener, local HTTP upstream, the production Workspace rule
repository/pipeline, Shift-JIS decoding, response assertions, capture headers, mutation and terminal
fault paths, and a selected Android UID traversing VpnService plus the proxy work together. The
